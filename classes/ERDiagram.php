...
class ERDiagram
{
	...
	public static function setup_drivers($database, $schema, $table)
	{
		...
		if (ERDiagram::check_diagrams_database_and_schema()) {
	...
	public static function load_current_schema()
	{
		...
		$sql = 'SELECT "table"."oid" AS "tableOID", 
			"table"."relname" AS "tableName", 
			COALESCE("pg_catalog"."obj_description"("table"."oid", \'pg_class\'), \'\') AS "tableComment", 
			"column"."attnum" AS "columnNum", 
			"column"."attname" AS "columnName", 
			"column"."attnotnull" AS "columnNotNull", 
			COALESCE(pg_get_expr("columnDef"."adbin", "columnDef"."adrelid"), \'\') AS "columnDefaultDef", 
			COALESCE("pg_catalog"."col_description"("column"."attrelid", "column"."attnum"), \'\') AS "columnComment", 
			UPPER("pg_catalog"."format_type"("column"."atttypid", "column"."atttypmod")) as "columnType"
	 		FROM "pg_catalog"."pg_class" AS "table"
			LEFT JOIN "pg_catalog"."pg_attribute" AS "column" 
			ON "table"."oid" = "column"."attrelid" 
			AND "column"."attnum" > 0 
			AND NOT "column"."attisdropped"
			LEFT JOIN "pg_catalog"."pg_attrdef" AS "columnDef" 
			ON "column"."attrelid" = "columnDef"."adrelid" 
			AND "column"."attnum" = "columnDef"."adnum"
			WHERE "relkind"=\'r\' 
	...
	public function validate()
	{
		$this->errors = array();
		if ($this->name == '') {
			$this->errors[] = 'strerdiagramneedsname';
		}
